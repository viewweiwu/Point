import { PerspectiveCamera, Scene, FogExp2, Fog, WebGLRenderer, Geometry, TextureLoader, Vector3, Vector2, VertexColors, Points, Color, PointsMaterial, Group } from 'three'
...
let camera, scene, renderer, particles, group, geometry, composer
...
function initVariable() {
  ...
  // 初始化相机
  camera = new PerspectiveCamera(75, window.innerWidth / window.innerHeight, 10, 5000)
  camera.position.z = 750
  // 初始化场景
  scene = new Scene()
  // 雾化
  scene.fog = new FogExp2(0x05050c, 0.001)
...
function initModel() {
  ...
  objLoader.load('assets/fish.obj', function (group, aa) {
    let bufferGeometry = group.children[0].geometry
    let geo = new Geometry().fromBufferGeometry(bufferGeometry)
    geo.center()
    geo.normalize()
